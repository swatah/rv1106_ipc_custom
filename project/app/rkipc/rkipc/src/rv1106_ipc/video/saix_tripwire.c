...
static RockIvaBaTaskParams g_tripwire_task_params;
...
int configure_rules_from_rkparam(RockIvaHandle handle) {
    memset(&g_tripwire_task_params, 0, sizeof(g_tripwire_task_params));
    RockIvaBaTaskParams* taskParams = &g_tripwire_task_params;
...
void ba_result_callback(const RockIvaBaResult* result, const RockIvaExecuteStatus status, void* userdata) {
    if (status != ROCKIVA_SUCCESS || !result) {
        printf("[Tripwire] Callback: Invalid status or result\n");
        return;
    }
    printf("[Tripwire] ba_result_callback called with %u objects\n", result->objNum);
    pthread_mutex_lock(&saix_event_callback_lock);
    SaixEventCallback cb = g_saix_event_callback;
    pthread_mutex_unlock(&saix_event_callback_lock);
    ...
    for (uint32_t i = 0; i < result->objNum; i++) {
        ...
        if (obj->triggerRulesNum > 0) {
            ...
            if (firstTrigger->triggerType == ROCKIVA_BA_RULE_CROSS) {
                ...
                // Determine diwrection using the rule configuration
                ...
                cb(firstTrigger->ruleID, "Tripwire", json_message);
                ...
                cb(firstTrigger->ruleID, "Area Invasion", json_message);
                printf(">>> ba_result_callback() triggered with %u objects\n", result->objNum);
...
int update_tripwire_config(void) {
    ...
    // Store current callback to preserve it (LOCKED)
    pthread_mutex_lock(&saix_event_callback_lock);
    ...
    // Restore the callback that might have been reset during configuration (LOCKED)
    pthread_mutex_lock(&saix_event_callback_lock);
...
int tripwire_process_frame(RockIvaBaResult *result) {
    if (!result || result->objNum == 0) {
        printf("[Tripwire] No objects to process\n");
        return -1;
    }
    ba_result_callback(result, ROCKIVA_SUCCESS, &g_tripwire_task_params);
    printf("[Tripwire] processing frame.\n");
    return 0;
}
